...
def model_gen(name, gpu, **kwargs):
    if name == 'node':
        ...
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        layer = NODElayer(NesterovNODE(DF(dim, nhid), None, use_h=True, sign=-1), algebraic_from_differential=True, differential_from_algebraic=True, time_requires_grad=False, evaluation_times=evaluation_times,  **kwargs)
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True))
