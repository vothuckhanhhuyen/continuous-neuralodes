        ...
        for run_idx in range(num_runs):
            ...
            if name == "node":
                ...
                feature_layers = [initial_velocity(data_dim, nhid), ODEBlock(NesterovNODEfunc(
                    data_dim, nhid, modelname="NesterovNODE"), 1, 2, tol=tol, half=True, algebraic_from_differential=True), Decoder(data_dim, 1)]
            elif name == "gnesterovnode":
                nhid = 14
                feature_layers = [initial_velocity(data_dim, nhid), ODEBlock(NesterovNODEfunc(
                    data_dim, nhid, modelname="GNesterovNODE", xi=args.xi, actv=hard_tanh_5), 1, 2, tol=tol, half=True, algebraic_from_differential=True, actv_k=hard_tanh_5, use_momentum=False), Decoder(data_dim, 1)]
