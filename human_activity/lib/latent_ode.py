	...
	def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, 
		...
		if isinstance(self.encoder_z0, Encoder_z0_ODE_RNN) or \
			...
			first_point_mu, first_point_std = self.encoder_z0(
				truth_w_mask, truth_time_steps, run_backwards = run_backwards, start_time=self.start_time)

			# print(first_point_mu.size())
			means_z0 = first_point_mu.repeat(n_traj_samples, 1, 1, 1)
			sigma_z0 = first_point_std.repeat(n_traj_samples, 1, 1, 1)
			# print(first_point_mu.size())
			# print(means_z0.size())
		...
		if self.node in ('HBNODE', 'GHBNODE', 'NesterovNODE', 'GNesterovNODE'):	
			sol_y = sol_y[:, :, :, 0, :]
