...
def create_LatentODE_model(args, input_dim, z0_prior, obsrv_std, device, 
	classif_per_tp = False, n_labels = 1, actv=nn.Tanh(), activation_h=nn.Tanh()):
	...
	if args.z0_encoder == "odernn":
		...
		z0_diffeq_solver = DiffeqSolver(enc_input_dim, 
                                  ...
                                  args.solver, 
                                  args.latents, 
								  odeint_rtol = args.rtol, 
          						  odeint_atol = args.atol, 
                  				  device = device,
                        		  node=args.node)
		...
		encoder_z0 = Encoder_z0_ODE_RNN(n_rec_dims, 
                                  ...
                                  device = device,
                                  node=args.node,
   								  activation_h=activation_h).to(device)
	...
	diffeq_solver = DiffeqSolver(gen_data_dim, gen_ode_func, args.solver, args.latents, 
	...
	model = LatentODE(
		...
		n_labels = n_labels,
		train_classif_w_reconstr = (args.dataset == "activity"),
